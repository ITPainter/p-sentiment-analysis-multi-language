# -*- coding: utf-8 -*-
"""
🇺🇸 영어 감성분석 (English Sentiment Analysis)
Google Colab에서 실행 가능한 영어 텍스트 감성분석 시스템

📋 기능:
- Excel 파일에서 영어 텍스트 읽기
- 영어 전용 감성분석 모델 사용
- 결과를 Excel 파일로 저장
...
def find_text_column(df):
    """텍스트 컬럼 찾기"""
    text_columns = ['comment', 'review', 'text', 'comments', 'content', 'message', 'post', 'tweet']
    text_column = None
...
def create_visualizations(results_df):
    ...
    plt.title('English Text Sentiment Analysis Results', fontsize=16, fontweight='bold')
    plt.xlabel('Sentiment', fontsize=12)
    plt.ylabel('Text Count', fontsize=12)
    ...
    plt.title('Sentiment Distribution Ratio', fontsize=16, fontweight='bold')
    plt.axis('equal')
    plt.show()
    
    # 3. 신뢰도 분포 히스토그램
    plt.figure(figsize=(10, 6))
    plt.hist(results_df['sentiment_confidence'], bins=20, alpha=0.7, color='lightgreen')
    plt.title('Sentiment Analysis Confidence Distribution', fontsize=16, fontweight='bold')
    ...
    if positive_texts:
        ...
        if processed_text.strip():
            ...
            plt.axis('off')
            plt.title('Positive Text WordCloud', fontsize=20, fontweight='bold')
            plt.tight_layout()
            plt.show()
...
def save_results(df, results_df, original_filename):
    ...
    # 결과 파일명 생성
    output_filename = f"english_sentiment_analysis_result_{pd.Timestamp.now().strftime('%Y%m%d_%H%M%S')}.xlsx"
    
    # Excel 파일로 저장
    with pd.ExcelWriter(output_filename, engine='openpyxl') as writer:
        # 전체 결과
        final_df.to_excel(writer, sheet_name='Sentiment_Analysis_Results', index=False)
        
        # 감성별 요약
        sentiment_summary = results_df['sentiment_label'].value_counts().reset_index()
        sentiment_summary.columns = ['Sentiment', 'Count']
        sentiment_summary['Percentage(%)'] = (sentiment_summary['Count'] / len(results_df) * 100).round(1)
...
def main():
    """메인 실행 함수"""
    print("🇺🇸 English Sentiment Analysis System Started!")
    print("=" * 50)
    
    # 1. 모델 로드
    tokenizer, model, device = load_english_model()
    ...
    if texts:
        ...
        print(f"✅ Sentiment analysis completed: {len(results_df)} texts analyzed")
        ...
        # 7. 요약
        print("\n" + "=" * 50)
        print("🎯 English Sentiment Analysis Completed!")
        print(f"📊 Total texts: {len(results_df)}")
        print(f"📊 Average confidence: {results_df['sentiment_confidence'].mean():.3f}")
        print(f"📁 Download file: {output_filename}")
        print("=" * 50)
